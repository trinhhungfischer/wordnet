...
const isNodeCryptic = (node: Node, crypticBubblesList: any[], edges: Edge[], nodes: Node[]) => {
  if (!crypticBubblesList || crypticBubblesList.length === 0) return false;
  const label = String(node.data.label).toLowerCase();
  
  if (crypticBubblesList.some((c: any) => c.word.toLowerCase() === label)) return true;
  
  if (node.data.isChunk) {
    const parentEdge = edges.find(e => e.target === node.id);
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode && crypticBubblesList.some((c: any) => c.word.toLowerCase() === String(parentNode.data.label).toLowerCase())) {
        return true;
      }
    }
  } else if (!node.data.isCategory) {
    ...
    const chunkLabels = childEdges
      ...
      .filter(child => child && child.data.isChunk)
      .map(child => String(child!.data.label).toLowerCase());
    if (chunkLabels.some(cLabel => crypticBubblesList.some((c: any) => c.word.toLowerCase() === cLabel))) {
      return true;
    }
  }
  return false;
...
const isNodeBackward = (node: Node, backwardBubblesList: any[], edges: Edge[], nodes: Node[]) => {
  if (!backwardBubblesList || backwardBubblesList.length === 0) return false;
  const label = String(node.data.label).toLowerCase();
  
  if (backwardBubblesList.some((f: any) => f.word.toLowerCase() === label)) return true;
  
  if (node.data.isChunk) {
    const parentEdge = edges.find(e => e.target === node.id);
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode && backwardBubblesList.some((f: any) => f.word.toLowerCase() === String(parentNode.data.label).toLowerCase())) {
        return true;
      }
    }
  } else if (!node.data.isCategory) {
    ...
    const chunkLabels = childEdges
      ...
      .filter(child => child && child.data.isChunk)
      .map(child => String(child!.data.label).toLowerCase());
    if (chunkLabels.some(cLabel => backwardBubblesList.some((f: any) => f.word.toLowerCase() === cLabel))) {
      return true;
    }
  }
  return false;
...
export const isNodeSpikeBubble = (node: Node, list: any[], edges: Edge[], nodes: Node[]) => {
  if (!list || list.length === 0) return false;
  const label = String(node.data.label).toLowerCase();
  if (list.some((f: any) => (typeof f === 'string' ? f : f.word).toLowerCase() === label)) return true;
  if (node.data.isChunk) {
    const parentEdge = edges.find(e => e.target === node.id);
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode && list.some((f: any) => (typeof f === 'string' ? f : f.word).toLowerCase() === String(parentNode.data.label).toLowerCase())) return true;
    }
  } else if (!node.data.isCategory) {
    const chunkLabels = edges.filter(e => e.source === node.id)
      .map(e => nodes.find(child => child.id === e.target))
      .filter(child => child && child.data.isChunk)
      .map(child => String(child!.data.label).toLowerCase());
    if (chunkLabels.some(cLabel => list.some((f: any) => (typeof f === 'string' ? f : f.word).toLowerCase() === cLabel))) return true;
  }
  return false;
};

const isNodeSoapBubble = (node: Node, list: any[], edges: Edge[], nodes: Node[]) => {
  if (!list || list.length === 0) return false;
  const label = String(node.data.label).toLowerCase();
  if (list.some((f: any) => f.word.toLowerCase() === label)) return true;
  if (node.data.isChunk) {
    const parentEdge = edges.find(e => e.target === node.id);
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode && list.some((f: any) => f.word.toLowerCase() === String(parentNode.data.label).toLowerCase())) return true;
    }
  } else if (!node.data.isCategory) {
    const chunkLabels = edges.filter(e => e.source === node.id)
      .map(e => nodes.find(child => child.id === e.target))
      .filter(child => child && child.data.isChunk)
      .map(child => String(child!.data.label).toLowerCase());
    if (chunkLabels.some(cLabel => list.some((f: any) => f.word.toLowerCase() === cLabel))) return true;
  }
};

const isNodeBombCrackingBubble = (node: Node, list: any[], edges: Edge[], nodes: Node[]) => {
  if (!list || list.length === 0) return { isBombCrackingBubble: false, mergeRemain: 0 };
  const label = String(node.data.label).toLowerCase();
  let b = list.find((f: any) => f?.word?.toLowerCase() === label);
  if (b) return { isBombCrackingBubble: true, mergeRemain: b.mergeRemain };
  if (node.data.isChunk) {
    const parentEdge = edges.find(e => e.target === node.id);
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
  ...
  return { isBombCrackingBubble: false, mergeRemain: 0 };
};

const isNodeFloatBubble = (node: Node, list: any[], edges: Edge[], nodes: Node[]) => {
  if (!list || list.length === 0) return { isFloatBubble: false, mergesToFloat: 0 };
  const label = String(node.data.label).toLowerCase();
  let b = list.find((f: any) => f?.word?.toLowerCase() === label);
...
export const isNodeResize = (node: Node, list: any[], edges: Edge[], nodes: Node[]) => {
  if (!list || list.length === 0) return false;
  const label = String(node.data.label).toLowerCase();
  if (list.some((f: any) => (typeof f === 'string' ? f : f.word).toLowerCase() === label)) return true;
  if (node.data.isChunk) {
...
export const isNodeScrewLock = (node: Node, screws: any[], edges: Edge[], nodes: Node[]) => {
  if (!screws || screws.length === 0) return -1;
  const label = String(node.data.label).toLowerCase();
  
  let idx = screws.findIndex((s: any) => s.screwLockWord.toLowerCase() === label);
  if (idx !== -1) return idx;
  ...
  if (node.data.isChunk) {
    ...
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode) {
        idx = screws.findIndex((s: any) => s.screwLockWord.toLowerCase() === String(parentNode.data.label).toLowerCase());
        if (idx !== -1) return idx;
      }
    }
  } else if (!node.data.isCategory) {
    ...
    for (const cLabel of chunkLabels) {
      idx = screws.findIndex((s: any) => s.screwLockWord.toLowerCase() === cLabel);
      if (idx !== -1) return idx;
    }
  }
  return -1;
...
export const isNodeScrewDriver = (node: Node, screws: any[], edges: Edge[], nodes: Node[]) => {
  ...
  let idx = screws.findIndex((s: any) => s.screwDriverWords.some((sdw: string) => sdw.toLowerCase() === label));
  if (idx !== -1) return idx;
  ...
  if (node.data.isChunk) {
    ...
    if (parentEdge) {
      const parentNode = nodes.find(n => n.id === parentEdge.source);
      if (parentNode) {
        idx = screws.findIndex((s: any) => s.screwDriverWords.some((sdw: string) => sdw.toLowerCase() === String(parentNode.data.label).toLowerCase()));
        if (idx !== -1) return idx;
      }
    }
  } else if (!node.data.isCategory) {
    ...
    for (const cLabel of chunkLabels) {
      idx = screws.findIndex((s: any) => s.screwDriverWords.some((sdw: string) => sdw.toLowerCase() === cLabel));
      if (idx !== -1) return idx;
    }
  }
  return -1;
...
export default function GraphEditor() {
  ...
  const [, setHistory] = useState<{ past: { nodes: Node[], edges: Edge[], rawData?: any }[], future: { nodes: Node[], edges: Edge[], rawData?: any }[] }>({ past: [], future: [] });
  ...
  const saveHistory = useCallback(() => {
    setHistory(h => {
      const newPast = [...h.past, { 
        ...
        rawData: rawLevelData ? JSON.parse(JSON.stringify(rawLevelData)) : null
      }].slice(-50);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
        ...
        setHistory(h => {
          ...
          setNodes(prev.nodes);
          setEdges(prev.edges);
          if (prev.rawData !== undefined) setRawLevelData(prev.rawData);
          return {
        ...
        setHistory(h => {
          ...
          setNodes(next.nodes);
          setEdges(next.edges);
          if (next.rawData !== undefined) setRawLevelData(next.rawData);
          return {
            past: [...h.past, { 
              nodes: JSON.parse(JSON.stringify(nodes)), 
              edges: JSON.parse(JSON.stringify(edges)),
  ...
  const handleRenameNode = (nodeId: string, newLabel: string) => {
    ...
    if (rawLevelData && oldLabel !== newLabelLower) {
      ...
      if (clonedRawData.soapBubbles) {
        clonedRawData.soapBubbles = clonedRawData.soapBubbles.map((sb: any) => 
          sb.word.toLowerCase() === oldLabel ? { ...sb, word: newLabelLower } : sb
        );
        isChanged = true;
      }

      if (clonedRawData.bombCrackingBubbles) {
        clonedRawData.bombCrackingBubbles = clonedRawData.bombCrackingBubbles.map((bcb: any) => 
          bcb.word.toLowerCase() === oldLabel ? { ...bcb, word: newLabelLower } : bcb
        );
        isChanged = true;
      }

      if (clonedRawData.floatBubbles) {
        clonedRawData.floatBubbles = clonedRawData.floatBubbles.map((fb: any) => 
          fb.word.toLowerCase() === oldLabel ? { ...fb, word: newLabelLower } : fb
        );
        isChanged = true;
      }
      ...
      if (clonedRawData.resizeBubbles) {
        clonedRawData.resizeBubbles = clonedRawData.resizeBubbles.map((rb: any) => 
          (typeof rb === 'string' ? rb : rb.word).toLowerCase() === oldLabel ? newLabelLower : rb
        );
        isChanged = true;
      }
    ...
    if (replaceNodes.length > 0) {
      ...
      oldWordNodes.forEach((oldNode, i) => {
        ...
        if (updatedRawLevelData) {
          ...
          // 4.3. Soap Bubble
          if (updatedRawLevelData.soapBubbles) {
            updatedRawLevelData.soapBubbles = updatedRawLevelData.soapBubbles.map((sb: any) => 
              sb.word.toLowerCase() === oldLabel ? { ...sb, word: newLabel } : sb
            );
          }

          if (updatedRawLevelData.bombCrackingBubbles) {
            updatedRawLevelData.bombCrackingBubbles = updatedRawLevelData.bombCrackingBubbles.map((bcb: any) => 
              bcb?.word?.toLowerCase() === oldLabel ? { ...bcb, word: newLabel } : bcb
            );
          }

          if (updatedRawLevelData.resizeBubbles) {
            updatedRawLevelData.resizeBubbles = updatedRawLevelData.resizeBubbles.map((rb: any) => 
              (typeof rb === 'string' ? rb : rb.word).toLowerCase() === oldLabel ? newLabel : rb
            );
          }

          if (updatedRawLevelData.floatBubbles) {
            updatedRawLevelData.floatBubbles = updatedRawLevelData.floatBubbles.map((fb: any) => 
              fb?.word?.toLowerCase() === oldLabel ? { ...fb, word: newLabel } : fb
            );
          }
          ...
          if (updatedRawLevelData.crypticBubbles) {
            updatedRawLevelData.crypticBubbles = updatedRawLevelData.crypticBubbles.map((cb: any) => {
              if (cb.word.toLowerCase() === oldLabel) {
                const newRevealAtMerge = new Array(newLabel.length).fill(0);
                const oldReveal = cb.revealAtMerge || [];
                for (let i = 0; i < Math.min(newLabel.length, oldReveal.length); i++) {
                  newRevealAtMerge[i] = oldReveal[i];
                }
                // Also clean up old `letters` if it existed
                const { letters, ...rest } = cb;
                return { ...rest, word: newLabel, revealAtMerge: newRevealAtMerge };
              }
              return cb;
            });
          }
      ...
      const importBranch = (catName: string, parentId: string | null, depth: number, currentReqSig: any) => {
        ...
        wordsToImport.forEach((w: any, index: number) => {
          ...
          if (oldWordNode && clonedRawData) {
            ...
            if (clonedRawData.soapBubbles) {
              clonedRawData.soapBubbles = clonedRawData.soapBubbles.map((sb: any) => 
                sb.word.toLowerCase() === oldLabel ? { ...sb, word: w.word } : sb
              );
            }

            if (clonedRawData.bombCrackingBubbles) {
              clonedRawData.bombCrackingBubbles = clonedRawData.bombCrackingBubbles.map((bcb: any) => 
                bcb?.word?.toLowerCase() === oldLabel ? { ...bcb, word: w.word } : bcb
              );
            }
            ...
            if (clonedRawData.resizeBubbles) {
              clonedRawData.resizeBubbles = clonedRawData.resizeBubbles.map((rb: any) => 
                (typeof rb === 'string' ? rb : rb.word).toLowerCase() === oldLabel ? w.word : rb
              );
            }
            ...
            if (clonedRawData.crypticBubbles) {
              clonedRawData.crypticBubbles = clonedRawData.crypticBubbles.map((cb: any) => {
                if (cb.word.toLowerCase() === oldLabel) {
                  const newRevealAtMerge = new Array(w.word.length).fill(0);
                  const oldReveal = cb.revealAtMerge || [];
                  for (let i = 0; i < Math.min(w.word.length, oldReveal.length); i++) {
                    newRevealAtMerge[i] = oldReveal[i];
                  }
                  const { letters, ...rest } = cb;
                  return { ...rest, word: w.word, revealAtMerge: newRevealAtMerge };
                }
                return cb;
              });
            }
        ...
        <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '4px', borderRadius: '10px', flexShrink: 0 }}>
            ...
            <button 
              ...
              style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid #3b82f6', background: 'rgba(59, 130, 246, 0.1)', color: '#60a5fa', cursor: 'pointer', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}
            >
            ...
            <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '6px', paddingRight: '4px' }}>
              {(sortLinksFirst 
                ...
                .map((nodeId: string) => {
                  ...
                  const isResize = isNodeResize(node, rawLevelData?.resizeBubbles || [], edges, nodes);
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: '13px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          {isSoapBubble && (
                            <CircleDashed size={12} color="#ec4899" />
                          )}
                          {isSpikeBubble && (
                            <Asterisk size={12} color="#dc2626" />
                          )}
                          {isCycleFadeOut && (
                          ...
                          {isBombCrackingBubble && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#f97316' }}>
                              <Flame size={12} />
                              <span style={{ fontSize: '10px' }}>{bombMergeRemain}</span>
                            </div>
                          )}
                          {isFloatBubble && (
                          ...
                          {screwLockIndex !== -1 && (
                            <Wrench size={12} color={lockKeyColors[screwLockIndex % lockKeyColors.length]} />
                          )}
                          {screwDriverIndex !== -1 && (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          {screwLockIndex !== -1 && <Wrench size={14} color={selectedNodeId === nodeId ? "white" : lockKeyColors[screwLockIndex % lockKeyColors.length]} />}
                          ...
                          {isSoapBubble && <CircleDashed size={14} color={selectedNodeId === nodeId ? "white" : "#ec4899"} />}
                          {isBombCrackingBubble && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px', color: selectedNodeId === nodeId ? "white" : "#f97316" }}>
                              <Flame size={14} />
                              <span style={{ fontSize: '11px' }}>{bombMergeRemain}</span>
                            </div>
                          )}
                          {isFloatBubble && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px', color: selectedNodeId === nodeId ? "white" : "#60a5fa" }}>
                              <Cloud size={14} />
                              <span style={{ fontSize: '11px' }}>{mergesToFloat}</span>
                            </div>
                          )}
                          {isSpikeBubble && <Asterisk size={14} color={selectedNodeId === nodeId ? "white" : "#dc2626"} />}
      ...
      <ReactFlow
        nodes={nodes.map(n => ({
          ...
          data: {
            ...
            isBombCrackingBubble: isNodeBombCrackingBubble(n, rawLevelData?.bombCrackingBubbles || [], edges, nodes).isBombCrackingBubble,
            ...
            isResize: isNodeResize(n, rawLevelData?.resizeBubbles || [], edges, nodes),
