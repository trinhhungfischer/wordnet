...
export default function SolutionModal({ isOpen, onClose, nodes, edges, levelData, spawnQueueIds }: SolutionModalProps) {
  ...
  const displayNodes = useMemo(() => {
    const list = activeBoardState.map((bubbleState: any, idx: number) => {
      ...
      let isCrackBubble = bubbleState.isCrackBubble;
      let crackCountRemaining = bubbleState.crackCountRemaining;
          ...
          <div style={{ flex: 2, padding: '20px', background: 'rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', paddingRight: '4px' }}>
              {displayNodes.length === 0 ? (
                ...
                displayNodes.map(({ nodeId, node, isTemp, displayLabel, isChunk, isCategory, familyId, isChained, chainMergesLeft, iceMergesLeft, crackMergesLeft, isCrackBubble, crackCountRemaining, burstMovesRemaining, lockIndex, keyIndex, screwCount, isScrewDriver, screwLockIndex, screwDriverIndex, cycleLockState, isImmovable, countdownValue, isLinkedMain, isLinkedChunk, reqLockWeight, originalIdx, isIceBomb, iceBombTurnToActive, iceBombInfectedFreezeTurns, isSpikeBubble, isBombCrackingBubble, bombMergeRemain, isFloatBubble, mergesToFloat, isTeleportBubble, mergesToTeleport, stackPipeId, stackPipeDepth, isResize }: any) => {
                  ...
                  const baseBg = isChunk ? 'rgba(99,102,241,0.05)' : 'rgba(255,255,255,0.05)';
                  let bgColor = useFamilyColor ? familyBg : baseBg;
                  if (!useFamilyColor) {
                    bgColor = isIceBomb ? 'rgba(56, 189, 248, 0.15)' :
                              ...
                              isTeleportBubble ? 'rgba(234, 179, 8, 0.15)' :
                              isResize ? 'rgba(245, 158, 11, 0.15)' :
                              stackPipeId !== undefined ? 'rgba(16, 185, 129, 0.15)' : baseBg;
                  }

                  const baseBorder = isChunk ? '1px solid rgba(99,102,241,0.3)' : '1px solid var(--panel-border)';
                  let borderColor = useFamilyColor ? familyBorder : baseBorder;
                  if (!useFamilyColor) {
                    borderColor = reqLockWeight !== undefined ? '1px solid #f97316' :
                                  isIceBomb ? '1px solid rgba(56, 189, 248, 0.4)' :
                                  ...
                                  isTeleportBubble ? '1px dashed rgba(234, 179, 8, 0.8)' : 
                                  isResize ? '1px dashed rgba(245, 158, 11, 0.8)' : 
                                  stackPipeId !== undefined ? '1px solid rgba(16, 185, 129, 0.4)' : baseBorder;
                  }
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          {isIceBomb && (
                            <span title={`Ice Bomb (${iceBombTurnToActive} merges left)`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#38bdf8', fontSize: '11px', fontWeight: 'bold' }}>
                              <Radiation size={14} color="#38bdf8" /> {iceBombTurnToActive}
                            </span>
                          )}
                          ...
                          {reqLockWeight !== undefined && (
                            <span title={`Requires ${reqLockWeight} words to merge`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#f97316', fontSize: '11px', fontWeight: 'bold' }}>
                              <Dumbbell size={14} color="#f97316" /> {reqLockWeight}
                            </span>
                          )}
                          {isImmovable && (
                          ...
                          {isCrackBubble && (
                            <span title={`Crack Bubble (${crackCountRemaining} hits left)`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#fbbf24', fontSize: '11px', fontWeight: 'bold' }}>
                              <Zap size={14} color="#fbbf24" /> {crackCountRemaining}
                            </span>
                          )}
                          {isBackward && (
                          ...
                          {isTeleportBubble && mergesToTeleport !== undefined && (
                            <span title={`Teleport Bubble (${mergesToTeleport} merges to teleport)`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#eab308', fontSize: '11px', fontWeight: 'bold' }}>
                              <Rocket size={14} color="#eab308" /> {mergesToTeleport}
                            </span>
                          )}
                          {isResize && (
                            <span title={`Resize Bubble`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#f59e0b', fontSize: '11px', fontWeight: 'bold' }}>
                              <Maximize size={14} color="#f59e0b" /> Resize
                            </span>
                          )}
                          {stackPipeId !== undefined && (
                            <span title={`Stack Pipe #${stackPipeId} (Depth: ${stackPipeDepth})`} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: '#10b981', fontSize: '11px', fontWeight: 'bold' }}>
                              <Layers size={14} color="#10b981" /> {stackPipeDepth !== undefined ? stackPipeDepth + 1 : ''}
                            </span>
                          )}
