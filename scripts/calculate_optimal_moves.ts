...
function runAnalysis() {
  ...
  // Using a header similar to real_levels_difficulty_v4.csv but adding Min Recommended Limit
  results.push('Level,Total Nodes,Spawn Queue Length,Optimal Moves,Bonus Turns,Min Recommended Limit,Config Move Limit,Peak Congestion,Congestion Turns,Rarity Score,Mechanics Count,Move Tightness,Total Categories');
  ...
  for (let i = 1; i <= 1000; i++) {
    ...
    if (fs.existsSync(filePath)) targetFile = filePath;
      ...
      console.warn(`Level ${i} not found, skipping.`);
      continue;
    }
    ...
    try {
      ...
      const minRecommendedLimit = res.recommendedMoveLimit;
      const optimalMoves = res.totalMoves;
      ...
      const moveTightness = configMoveLimit - minRecommendedLimit;
      
      const totalCategories = data.categories ? data.categories.length : 0;
      
      results.push(`${i},${nodes.length},${spawnQueueIds.length},${optimalMoves},${res.bonusTurns},${minRecommendedLimit},${configMoveLimit},${peakCongestion},${congestionTurns},${rarityScore},${mechanicsCount},${moveTightness},${totalCategories}`);
      
      if (i % 100 === 0) {
        console.log(`Processed up to level ${i}`);
      }
    } catch (e: any) {
  ...
  // Create folder analysis_scripts/YYYY-MM-DD
  const today = new Date().toISOString().split('T')[0];
  const outputDir = path.join(process.cwd(), 'analysis_scripts', today);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  const outPath = path.join(outputDir, 'Optimal_Moves_1_1000.csv');
